...
- `scripts/build-search-index.mjs` runs as part of `npm run build`. It splits the English text of every chapter into ~90-word passages, embeds them with `Xenova/all-MiniLM-L6-v2` (via `@huggingface/transformers`, a devDependency), and writes `static/search-passages.json` and `static/search-vectors.bin`. Both are generated and gitignored.
